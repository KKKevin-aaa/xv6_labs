...
void sbrkfail(char *s) {
    ...
    for (i = 0; i < sizeof(pids) / sizeof(pids[0]); i++) {
        if ((pids[i] = fork()) == 0) {
            ...
            if (sbrk(BIG - (uint64)sbrk(0)) == (char *)SBRK_ERROR) write(fds[1], "0", 1);
            else
                write(fds[1], "1", 1);
