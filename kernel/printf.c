...
int load_debug_sym(){
    ...
    if(alloc_str_len & (alloc_str_len-1)){ //Consider Buddy-system
        ...
        alloc_str_len=1ull<<msb;    //Align up
    }
    void *mem0=NULL;
...
void find_debug_info(uint64 pa){
    if(pa<KERNBASE || pa>=PHYSTOP){
        ...
        printf("Unable convert into kernel address:0x%lx\n", pa);
        return;
    }
...
void backtrace(){
    ...
    printf("(Kernel stack range from 0x%lx to 0x%lx)\n", stack_start, stack_end);
    while(1){
        if(cur_s0==0){
        ...
        if(safe_load_data(cur_s0-8, &cur_func_ra, stack_start, stack_end)!=1){
            ...
            else    printf("[Corrupted Stack frame](0x%lx)\n", cur_s0-8);
            return;
        }
